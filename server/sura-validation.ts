...
export type CommerceOrderStatus = "draft" | "awaiting_payment" | "paid" | "processing" | "delivered" | "cancelled" | "refunded";

export function isValidCommerceOrderStatusTransition(from: CommerceOrderStatus, to: CommerceOrderStatus) {
...
export const companyMemberRoles = ["owner", "manager", "editor"] as const;
export type CompanyMemberRole = (typeof companyMemberRoles)[number];

const companyRoleRank: Record<CompanyMemberRole, number> = { owner: 3, manager: 2, editor: 1 };

export function assertCompanyRoleAtLeast(role: CompanyMemberRole | undefined, minimum: CompanyMemberRole) {
  if (!role) throw new Error("You need a company membership to manage this studio");
  if (companyRoleRank[role] < companyRoleRank[minimum]) {
    const labels: Record<CompanyMemberRole, string> = { owner: "the company owner", manager: "a company manager", editor: "a company editor" };
    throw new Error(`This action requires ${labels[minimum]}`);
  }
}
