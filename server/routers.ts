...
import { z } from "zod";
import { addCompanyMember, completeAiAssistRequest, createAiAssistRequest, createBuildShareRecord, createCompanyForUser, createCompanyPost, createCompanyProduct, createCommerceOrder, createDeliveryQuote, createDiscountOffer, createInquiryRecord, createPaymentOrder, createPlatformAnnouncement, createVerifiedReview, failAiAssistRequest, getAccountProfile, getActiveAuthVisualSet, getAdminCommerceOverview, getAdminCompanyPostReviewQueue, getAdminCompanyReviewQueue, getAdminDiscountReviewQueue, getAdminReviewQueue, getAestheticPreferences, getAiAssistRequestForUser, getBoardSelections, getBuildShareRecord, getCompaniesForUser, getCompanyContacts, getCompanyMembers, getCompanyMembership, getCompanyOwnedByUser, getCompanyPostsForOwner, getCompanyProduct, getCompanyProducts, getCompanyStudioOverview, getCompanyViews, getCommerceOrdersForCompany, getCommerceOrdersForUser, getCompanySocialSummary, getDiscountOffersForCompany, getInquiriesForCompany, getOrCreateFreeMembership, getPaymentOrdersForUser, getProductById, getPublicAccountProfile, getPublicCompanyProfile, getPublicCompanyPosts, getPublicDiscountOffers, getPublicPlatformContacts, getPublicProducts, getSavedVendorIds, getSocialFeed, getUserSocialSummary, getVerifiedReviewsForCompany, getWebNotificationFeed, markWebNotificationRead, recordAiImageConsent, recordContentView, recordLegalConsent, removeCompanyMember, publishAuthVisualSet, replaceCompanyContacts, replacePlatformContacts, setAestheticPreferences, toggleBuildBoardSelection, toggleCompanyFollow, togglePostLike, togglePostRepost, toggleSavedVendor, toggleUserFollow, updateCompanyCommissionRate, updateCompanyMemberRole, updateCompanyPostStatus, updateCompanyReviewStatus, updateCommerceOrderStatus, updateDiscountOfferReviewStatus, updateInquiryStatus, updateVerifiedReviewStatus, upsertAccountProfile } from "./db";
...
import { accountProfileInputSchema, aestheticPreferencesInputSchema, announcementInputSchema, assertCompanyPaymentOwnership, assertCompanyRoleAtLeast, companyContactsInputSchema, companyCreateInputSchema, contactInputSchema, discountOfferInputSchema, legalConsentInputSchema, paymentCatalog, paymentOrderInputSchema } from "./sura-validation";
...
async function requireCompanyMember(userId: number, companyId: number, minRole: "owner" | "manager" | "editor" = "editor") {
  const membership = await getCompanyMembership(companyId, userId);
  if (!membership) throw new Error("You need a company membership to manage this studio");
  assertCompanyRoleAtLeast(membership.memberRole, minRole);
  return membership;
}
...
export const appRouter = router({
  ...
  system: systemRouter,
  features: router({
    state: publicProcedure.query(() => getFeatureState()),
  }),
  public: router({
  ...
  commerce: router({
    ...
    trackView: publicProcedure.input(z.object({ contentType: z.enum(["post", "product", "company_profile"]), contentId: z.number().int().positive(), companyId: z.number().int().positive() })).mutation(({ ctx, input }) => recordContentView({ ...input, viewerUserId: ctx.user?.id })),
  ...
  companies: router({
    ...
    overview: protectedProcedure.input(z.object({ companyId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "manager");
      return getCompanyStudioOverview(input.companyId);
    }),
    views: protectedProcedure.input(z.object({ companyId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "manager");
      return getCompanyViews(input.companyId);
    }),
    ...
    updateOrderStatus: protectedProcedure.input(z.object({ companyId: z.number().int().positive(), orderId: z.number().int().positive(), to: z.enum(["draft", "awaiting_payment", "paid", "processing", "delivered", "cancelled", "refunded"]) })).mutation(async ({ ctx, input }) => {
      const membership = await requireCompanyMember(ctx.user.id, input.companyId, "manager");
      return updateCommerceOrderStatus({ companyId: input.companyId, orderId: input.orderId, to: input.to, updatedByUserId: ctx.user.id, requestedByRole: membership.memberRole });
    }),
    inquiries: protectedProcedure.input(z.object({ companyId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "manager");
      return getInquiriesForCompany(input.companyId);
    }),
    setInquiryStatus: protectedProcedure.input(z.object({ companyId: z.number().int().positive(), inquiryId: z.number().int().positive(), status: z.enum(["new", "reviewed", "contacted", "closed"]) })).mutation(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "manager");
      return updateInquiryStatus(input.companyId, input.inquiryId, input.status);
    }),
    reviews: protectedProcedure.input(z.object({ companyId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "manager");
      return getVerifiedReviewsForCompany(input.companyId);
    }),
    team: protectedProcedure.input(z.object({ companyId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "manager");
      return getCompanyMembers(input.companyId);
    }),
    addMember: protectedProcedure.input(z.object({ companyId: z.number().int().positive(), email: z.string().email(), role: z.enum(["owner", "manager", "editor"]).default("editor") })).mutation(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "owner");
      return addCompanyMember(input.companyId, input.email, input.role);
    }),
    setMemberRole: protectedProcedure.input(z.object({ companyId: z.number().int().positive(), userId: z.number().int().positive(), role: z.enum(["owner", "manager", "editor"]) })).mutation(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "owner");
      return updateCompanyMemberRole(input.companyId, input.userId, input.role);
    }),
    removeMember: protectedProcedure.input(z.object({ companyId: z.number().int().positive(), userId: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
      await requireCompanyMember(ctx.user.id, input.companyId, "owner");
      return removeCompanyMember(input.companyId, input.userId);
    }),
  }),
  payments: router({
  ...
  admin: router({
    ...
    commerceOverview: adminProcedure.query(() => getAdminCommerceOverview()),
    reviewQueue: adminProcedure.query(() => getAdminReviewQueue()),
    setReviewStatus: adminProcedure.input(z.object({ reviewId: z.number().int().positive(), status: z.enum(["published", "rejected"]) })).mutation(({ input }) => updateVerifiedReviewStatus(input.reviewId, input.status)),
