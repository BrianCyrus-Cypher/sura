...
export async function createInquiryRecord(input: typeof inquiries.$inferInsert) {
  ...
  if (input.companyId) {
    const [company] = await db.select({ ownerUserId: companies.ownerUserId, name: companies.name }).from(companies).where(eq(companies.id, input.companyId)).limit(1);
    if (company) await createWebNotification({ userId: company.ownerUserId, kind: "company", title: "A new inquiry arrived", body: `${input.name} asked about ${company.name} from ${input.city}.`, linkUrl: "/company" });
  }
...
export async function updateCompanyReviewStatus(companyId: number, verificationStatus: "draft" | "pending" | "verified" | "rejected") {
  ...
  const [company] = await db.select({ ownerUserId: companies.ownerUserId, name: companies.name }).from(companies).where(eq(companies.id, companyId)).limit(1);
  if (company) await createWebNotification({ userId: company.ownerUserId, kind: "platform", title: verificationStatus === "verified" ? "Your company was verified" : "Your company review updated", body: `${company.name} is now ${verificationStatus}.`, linkUrl: "/company" });
  return { persisted: true };
}
...
export async function createWebNotification(input: { userId: number; kind: "membership" | "offer" | "company" | "platform"; title: string; body: string; linkUrl?: string }) {
  const db = await getDb();
  if (!db) return { id: 0, persisted: false };
...
export async function createDeliveryQuote(input: { productId: number; destinationCity: string; distanceBand: "same_neighbourhood" | "same_city" | "regional" | "national"; deliveryKes: number; providerLabel: string; expiresAt: Date }) {
  const db = await getDb();
  if (!db) return { id: 0, persisted: false };
...
export async function createCommerceOrder(input: { userId: number; companyId: number; productId: number; deliveryQuoteId: number; quantity: number; merchandiseSubtotalKes: number; commissionRatePct: number; commissionKes: number; sellerSettlementKes: number; deliveryKes: number; customerTotalKes: number; destinationCity?: string }) {
  const db = await getDb();
  if (!db) return { id: 0, persisted: false };
  const [customer] = await db.select({ name: users.name, email: users.email }).from(users).where(eq(users.id, input.userId)).limit(1);
  const result = await db.insert(commerceOrders).values({
    ...input,
  ...
  const companyOwner = await db.select({ ownerUserId: companies.ownerUserId, name: companies.name }).from(companies).where(eq(companies.id, input.companyId)).limit(1);
  if (companyOwner?.[0]) await createWebNotification({ userId: companyOwner[0].ownerUserId, kind: "platform", title: `New order from ${customer?.name ?? "a customer"}`, body: `${customer?.name ?? "A customer"} placed an order for ${input.quantity} item${input.quantity === 1 ? "" : "s"} worth ${formatKesValue(input.customerTotalKes)}.`, linkUrl: `/company` });
  return { id: Number(result[0]?.insertId ?? 0), persisted: true };
}

export async function updateCommerceOrderStatus(input: { companyId: number; orderId: number; to: CommerceOrderStatus; updatedByUserId: number; requestedByRole: CompanyMemberRole }) {
  const db = await getDb();
  if (!db) return { persisted: false, previousStatus: null, status: input.to };
  assertCompanyRoleAtLeast(input.requestedByRole, "manager");
  const [order] = await db.select().from(commerceOrders).where(and(eq(commerceOrders.id, input.orderId), eq(commerceOrders.companyId, input.companyId))).limit(1);
  if (!order) throw new Error("This order does not belong to the company studio");
  if (!isValidCommerceOrderStatusTransition(order.status, input.to)) throw new Error(`Orders cannot move from ${order.status.replace("_", " ")} to ${input.to.replace("_", " ")}`);
  await db.update(commerceOrders).set({ status: input.to }).where(eq(commerceOrders.id, input.orderId));
  const [customer] = await db.select({ userId: commerceOrders.userId }).from(commerceOrders).where(eq(commerceOrders.id, input.orderId)).limit(1);
  if (customer && (input.to === "delivered" || input.to === "refunded")) {
    const [company] = await db.select({ name: companies.name }).from(companies).where(eq(companies.id, input.companyId)).limit(1);
    await createWebNotification({ userId: customer.userId, kind: "company", title: input.to === "delivered" ? "Your order has been delivered" : "Your order has been refunded", body: `${company?.name ?? "The company"} marked your order as ${input.to}.`, linkUrl: `/dashboard/orders` });
  }
  return { persisted: true, previousStatus: order.status, status: input.to };
}

export async function getCommerceOrdersForCompany(companyId: number) {
  const db = await getDb();
  if (!db) return [];
  const orders = await db.select().from(commerceOrders).where(eq(commerceOrders.companyId, companyId)).orderBy(desc(commerceOrders.createdAt)).limit(100);
  if (!orders.length) return [];
  const [products, reviews] = await Promise.all([
    db.select({ id: companyProducts.id, name: companyProducts.name, imageUrl: companyProducts.imageUrl, priceKes: companyProducts.priceKes }).from(companyProducts).where(inArray(companyProducts.id, orders.map((order) => order.productId))),
    db.select({ orderId: verifiedReviews.orderId, status: verifiedReviews.status, rating: verifiedReviews.rating }).from(verifiedReviews).where(eq(verifiedReviews.companyId, companyId)),
  ]);
  const productById = new Map(products.map((product) => [product.id, product]));
  const reviewByOrder = new Map(reviews.map((review) => [review.orderId, review]));
  return orders.map((order) => ({ ...order, product: productById.get(order.productId) ?? null, review: reviewByOrder.get(order.id) ?? null }));
}

export async function getInquiriesForCompany(companyId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(inquiries).where(eq(inquiries.companyId, companyId)).orderBy(desc(inquiries.createdAt)).limit(100);
}

export async function updateInquiryStatus(companyId: number, inquiryId: number, status: "new" | "reviewed" | "contacted" | "closed") {
  const db = await getDb();
  if (!db) return { persisted: false };
  const [inquiry] = await db.select({ id: inquiries.id }).from(inquiries).where(and(eq(inquiries.id, inquiryId), eq(inquiries.companyId, companyId))).limit(1);
  if (!inquiry) throw new Error("This inquiry does not belong to the company studio");
  await db.update(inquiries).set({ status }).where(eq(inquiries.id, inquiryId));
  return { persisted: true };
}

export async function getVerifiedReviewsForCompany(companyId: number) {
  const db = await getDb();
  if (!db) return [];
  const reviews = await db.select().from(verifiedReviews).where(eq(verifiedReviews.companyId, companyId)).orderBy(desc(verifiedReviews.createdAt)).limit(100);
  const [products, customers] = reviews.length ? await Promise.all([
    db.select({ id: companyProducts.id, name: companyProducts.name }).from(companyProducts).where(inArray(companyProducts.id, reviews.map((review) => review.productId))),
    db.select({ id: users.id, name: users.name }).from(users).where(inArray(users.id, reviews.map((review) => review.userId))),
  ]) : [[], []];
  ...
  return reviews.map((review) => ({ ...review, productName: productById.get(review.productId) ?? "Product", customerName: customerById.get(review.userId) ?? "Customer" }));
}

export async function recordContentView(input: { contentType: "post" | "product" | "company_profile"; contentId: number; companyId: number; viewerUserId?: number }) {
  const db = await getDb();
  if (!db) return { persisted: false };
  await db.insert(contentViews).values({ ...input, viewerUserId: input.viewerUserId ?? null });
  return { persisted: true };
}

export async function getCompanyViews(companyId: number, days = 28) {
  const db = await getDb();
  if (!db) return { totalViews: 0, postViews: 0, productViews: 0, profileViews: 0, recentDays: [] as Array<{ day: string; count: number }> };
  const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
  const rows = await db.select({ contentType: contentViews.contentType, count: count(contentViews.id) }).from(contentViews).where(and(eq(contentViews.companyId, companyId), sql`${contentViews.createdAt} >= ${since}`)).groupBy(contentViews.contentType);
  const dailyRows = await db.select({ day: sql`DATE(${contentViews.createdAt})`, count: count(contentViews.id) }).from(contentViews).where(and(eq(contentViews.companyId, companyId), sql`${contentViews.createdAt} >= ${since}`)).groupBy(sql`DATE(${contentViews.createdAt})`).orderBy(sql`DATE(${contentViews.createdAt})`);
  const byType = new Map(rows.map((row) => [row.contentType, Number(row.count)]));
  const postViews = byType.get("post") ?? 0;
  const productViews = byType.get("product") ?? 0;
  const profileViews = byType.get("company_profile") ?? 0;
  return {
    totalViews: postViews + productViews + profileViews,
    postViews,
    productViews,
    profileViews,
    recentDays: dailyRows.map((row) => ({ day: String(row.day), count: Number(row.count) })),
  };
}

export async function getCompanyStudioOverview(companyId: number) {
  const db = await getDb();
  if (!db) return { orderCount: 0, revenueKes: 0, deliveredCount: 0, customers: 0, followerCount: 0, reviewCount: 0, averageRating: 0, inquiryCount: 0 };
  const [orderRows, followerRows, reviewRows, inquiryRows] = await Promise.all([
    db.select({ id: commerceOrders.id, status: commerceOrders.status, userId: commerceOrders.userId, customerTotalKes: commerceOrders.customerTotalKes, commissionRatePct: commerceOrders.commissionRatePct }).from(commerceOrders).where(eq(commerceOrders.companyId, companyId)),
    db.select({ total: count(companyFollows.id) }).from(companyFollows).where(eq(companyFollows.companyId, companyId)),
    db.select({ total: count(verifiedReviews.id), avgRating: sql<number>`AVG(${verifiedReviews.rating})` }).from(verifiedReviews).where(eq(verifiedReviews.companyId, companyId)),
    db.select({ total: count(inquiries.id) }).from(inquiries).where(eq(inquiries.companyId, companyId)),
  ]);
  const settledOrders = orderRows.filter((order) => order.status !== "cancelled" && order.status !== "refunded" && order.status !== "awaiting_payment");
  const revenueKes = settledOrders.reduce((sum, order) => sum + (Number(order.customerTotalKes) - Math.round(Number(order.customerTotalKes) * Number(order.commissionRatePct) / 100)), 0);
  return {
    orderCount: orderRows.length,
    revenueKes: Math.round(revenueKes),
    deliveredCount: orderRows.filter((order) => order.status === "delivered").length,
    customers: new Set(orderRows.map((order) => order.userId)).size,
    followerCount: Number(followerRows[0]?.total ?? 0),
    reviewCount: Number(reviewRows[0]?.total ?? 0),
    averageRating: Number(reviewRows[0]?.avgRating ?? 0),
    inquiryCount: Number(inquiryRows[0]?.total ?? 0),
  };
}
...
export async function getCompanyMembers(companyId: number) {
  ...
  const rows = await db.select({
    ...
    name: users.name,
    email: users.email,
  }).from(companyMembers).innerJoin(users, eq(companyMembers.userId, users.id)).where(eq(companyMembers.companyId, companyId)).orderBy(desc(companyMembers.createdAt));
  return rows;
}

export async function addCompanyMember(companyId: number, email: string, role: CompanyMemberRole) {
  const db = await getDb();
  if (!db) return { id: 0, persisted: false };
  const user = await findUserByEmail(email.trim().toLowerCase());
  if (!user) throw new Error("No account uses that email yet");
  await db.insert(companyMembers).values({ companyId, userId: user.id, memberRole: role }).onDuplicateKeyUpdate({ set: { memberRole: role } });
  const [company] = await db.select({ name: companies.name }).from(companies).where(eq(companies.id, companyId)).limit(1);
  await createWebNotification({ userId: user.id, kind: "company", title: "You joined a company studio", body: `You are now a ${role} at ${company?.name ?? "this company"}.`, linkUrl: "/company" });
  return { id: user.id, persisted: true };
}

export async function updateCompanyMemberRole(companyId: number, targetUserId: number, role: CompanyMemberRole) {
  const db = await getDb();
  if (!db) return { persisted: false };
  await db.update(companyMembers).set({ memberRole: role }).where(and(eq(companyMembers.companyId, companyId), eq(companyMembers.userId, targetUserId)));
  return { persisted: true };
}

export async function removeCompanyMember(companyId: number, targetUserId: number) {
  const db = await getDb();
  if (!db) return { persisted: false };
  await db.delete(companyMembers).where(and(eq(companyMembers.companyId, companyId), eq(companyMembers.userId, targetUserId)));
  return { persisted: true };
}

export async function getAdminCommerceOverview() {
  const db = await getDb();
  if (!db) return { orderCount: 0, revenueKes: 0, deliveredCount: 0, awaitingCount: 0, companyCount: 0, userCount: 0, recentOrders: [] };
  const [orderRows, companyRows, userRows] = await Promise.all([
    db.select().from(commerceOrders).orderBy(desc(commerceOrders.createdAt)).limit(200),
    db.select({ total: count(companies.id) }).from(companies),
    db.select({ total: count(users.id) }).from(users),
  ]);
  return {
    orderCount: orderRows.length,
    revenueKes: orderRows.filter((order) => order.status !== "cancelled" && order.status !== "refunded").reduce((sum, order) => sum + Number(order.customerTotalKes), 0),
    deliveredCount: orderRows.filter((order) => order.status === "delivered").length,
    awaitingCount: orderRows.filter((order) => order.status === "awaiting_payment").length,
    companyCount: Number(companyRows[0]?.total ?? 0),
    userCount: Number(userRows[0]?.total ?? 0),
    recentOrders: orderRows.slice(0, 20).map((order) => ({ id: order.id, customerEmail: order.customerEmail, customerName: order.customerName, customerTotalKes: order.customerTotalKes, status: order.status, createdAt: order.createdAt, companyId: order.companyId, productId: order.productId })),
  };
}
...
export async function getAdminReviewQueue() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(verifiedReviews).where(eq(verifiedReviews.status, "pending")).limit(50);
}

export async function updateVerifiedReviewStatus(reviewId: number, status: "published" | "rejected") {
  const db = await getDb();
  if (!db) return { persisted: false };
  await db.update(verifiedReviews).set({ status }).where(eq(verifiedReviews.id, reviewId));
  return { persisted: true };
}
