...
function LocationProbe() {
  const { county, city, message, requestLocation, setCounty, status } = useKenyaLocation();
  return <div>
    <output data-testid="county">{county ?? "none"}</output>
    <output data-testid="city">{city ?? "none"}</output>
    <output data-testid="status">{status}</output>
    <output data-testid="message">{message ?? "none"}</output>
    <button onClick={() => setCounty("Kisumu")}>Choose Kisumu</button>
...
describe("KenyaLocationProvider", () => {
  it("persists a manually selected Kenyan county", () => {
    renderProbe();
    fireEvent.click(screen.getByText("Choose Kisumu"));
    expect(screen.getByTestId("county").textContent).toContain("Kisumu");
    expect(screen.getByTestId("city").textContent).toContain("Kisumu");
    expect(screen.getByTestId("status").textContent).toContain("manual");
    expect(window.localStorage.getItem("sura-kenya-county")).toBe("Kisumu");
    expect(window.localStorage.getItem("vibebuild-kenya-city")).toBe("Kisumu");
  });

  it("maps a county without a named city to city none", () => {
    renderProbe();
    fireEvent.click(screen.getByText("Choose Kisumu"));
    expect(screen.getByTestId("status").textContent).toContain("manual");
  });

  it("provides a manual-county fallback when browser location is unavailable", () => {
  ...
  it("handles denied permission and outside-Kenya coordinates without storing a location", () => {
    ...
    expect(screen.getByTestId("county").textContent).toContain("none");
  });
});