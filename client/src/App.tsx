...
import AiStudioTeaser from "@/pages/AiStudioTeaser";
...
function LandingGate() {
  ...
  if (loading) return <div style={{ backgroundColor: "var(--sura-page)" }} className="vb-paper min-h-screen" />;
  return isAuthenticated ? <Home /> : <LandingPage />;
}

function AiStudioGate() {
  return useFeature("ai_studio") ? <AiStudioPage /> : <AiStudioTeaser />;
}

function Router() {
  return (
    <Switch>
      <Route path="/">{() => <LandingGate />}</Route>
      ...
      <Route path="/ai-studio" component={AiStudioGate} />
