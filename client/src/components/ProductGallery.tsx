...
export function ProductGallery({ images, name, compact = false }: ProductGalleryProps) {
  ...
  if (!safeImages.length) {
    return <div className={`grid place-items-center bg-[var(--sura-soft)] ${compact ? "aspect-[4/3]" : "min-h-[25rem]"}`}><ImageIcon className="h-8 w-8 text-[var(--sura-accent)]" /></div>;
  }
  ...
  return <div className="space-y-3">
    <div className={`group relative overflow-hidden rounded-2xl bg-[var(--sura-soft)] outline outline-1 outline-[var(--sura-border)] outline-offset-[-1px] ${compact ? "aspect-[4/3]" : "min-h-[25rem] sm:min-h-[34rem]"}`}>
      ...
      {safeImages.length > 1 && <><div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/35 to-transparent" /><div className="absolute inset-x-3 bottom-3 flex items-end justify-between"><span className="rounded-full bg-[#171813]/75 px-2.5 py-1.5 text-[0.62rem] font-bold text-white backdrop-blur">{activeIndex + 1} / {safeImages.length}</span><div className="flex gap-1.5"><button onClick={() => move(-1)} className="vb-focus grid h-8 w-8 place-items-center rounded-full bg-[var(--sura-paper)]/90 text-[var(--sura-ink)] shadow-sm" aria-label="Previous product image"><ChevronLeft className="h-4 w-4" /></button><button onClick={() => move(1)} className="vb-focus grid h-8 w-8 place-items-center rounded-full bg-[var(--sura-paper)]/90 text-[var(--sura-ink)] shadow-sm" aria-label="Next product image"><ChevronRight className="h-4 w-4" /></button></div></div></>}
    </div>
    {safeImages.length > 1 && <div className="flex gap-2 overflow-x-auto pb-1" aria-label={`${name} image gallery`}>{safeImages.map((image, index) => <button key={`${image}-${index}`} onClick={() => setActiveIndex(index)} className={`vb-focus relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 bg-[var(--sura-soft)] ${activeIndex === index ? "" : "ransparent opacity-65 hover:opacity-100"}`} aria-label={`View ${name} image ${index + 1}`} aria-pressed={activeIndex === index}><SuraImage src={image} fallbackSrc="/assets/sura-auth-street.jpg" alt="" className="h-full w-full object-cover" /></button>)}</div>}
  </div>;
}
