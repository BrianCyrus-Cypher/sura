...
export default function LegalPage() {
  ...
  return <VibeLayout><Seo title={isPrivacy ? "Privacy Policy — SURA" : "Terms of Service — SURA"} description={isPrivacy ? "How SURA treats personal data, location choice, public profiles, and payment security." : "The platform terms that keep the SURA space clear for accounts, company spaces, directory information, and payment records."} /><main className="container max-w-4xl pb-20 pt-10 sm:pt-16"><Link href="/" className="vb-focus inline-flex items-center gap-2 text-sm font-bold text-[var(--sura-accent)]"><ArrowLeft className="h-4 w-4" />Back to SURA</Link><div className="mt-10 rounded-2xl border border-[var(--sura-border)] bg-[var(--sura-paper)] p-7 shadow-[0_18px_44px_rgba(60,42,22,0.06)] sm:p-12"><div className="flex items-start justify-between gap-4"><div><p className="vb-kicker text-[var(--sura-terracotta)]">{page.eyebrow}</p><h1 className="vb-serif mt-4 max-w-2xl text-5xl leading-[0.94] text-[var(--sura-ink)] sm:text-6xl">{page.title}</h1></div><LockKeyhole className="h-6 w-6 text-[var(--sura-sand)]" /></div><p className="mt-7 max-w-2xl text-base leading-7 text-[var(--sura-sand)]">{page.intro}</p><div className="mt-10    border-[var(--sura-border)]">{page.sections.map(([heading, copy]) => <section key={heading} className="py-6"><div className="flex gap-3"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[var(--sura-sand)]" /><div><h2 className="text-base font-bold text-[var(--sura-ink)]">{heading}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--sura-sand)]">{copy}</p></div></div></section>)}</div><p className="mt-8 text-xs leading-5 text-[var(--sura-sand)]">Draft version: 2026-08-22 · This is product copy, not legal advice.</p></div></main></VibeLayout>;
}
