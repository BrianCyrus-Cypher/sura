...
export const deliveryQuotes = mysqlTable(
  "delivery_quotes",
  {
    ...
    distanceBand: mysqlEnum("distanceBand", ["same_neighbourhood", "same_city", "regional", "national"]).notNull(),
...
export const commerceOrders = mysqlTable(
  "commerce_orders",
  {
    ...
    destinationCity: varchar("destinationCity", { length: 80 }),
...
export const contentViews = mysqlTable(
  "content_views",
  {
    id: int("id").autoincrement().primaryKey(),
    contentType: mysqlEnum("contentType", ["post", "product", "company_profile"]).notNull(),
  ...
  (table) => [
    index("content_views_company_idx").on(table.companyId),
    index("content_views_content_idx").on(table.contentType, table.contentId),
